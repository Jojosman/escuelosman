...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 18-03-2021 a las 20:41:08
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_delete_alumno` (IN `_ID_ALUMNO` INT(255))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_select_alumno_id` (IN `_ID_ALUMNO` INT(255))  BEGIN
SELECT
		id_alumno,
    	nombre,
    	apellido_paterno,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_select_datos_alumno` (IN `_ID_USUARIO` INT(255))  BEGIN
	SELECT
		id_alumno,
    	nombre,
    	apellido_paterno,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_select_login` (IN `_MATRICULA` VARCHAR(8), IN `_CURP` VARCHAR(18))  BEGIN
	SELECT
		id_usuario,
    	privilegio
    FROM usuarios
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_select_login_alumno` (IN `_MATRICULA` VARCHAR(8), IN `_CURP` VARCHAR(18))  BEGIN
	SELECT
	id_alumno
    FROM alumnos
    WHERE Matricula = _MATRICULA
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_select_search_alumno` (IN `_PATERNO` VARCHAR(255))  BEGIN
SELECT
		id_alumno,
    	nombre,
    	apellido_paterno,
...
(8, NULL, 'Diego', 'Landaverde', 'Briones', 'diego@gmail.com', '2000-06-21', 'LABD000621HCLNRGA5', '18005433', 1, 1, 'femenino', '8781547118', '8781547118', 'Colinas 3 Azucena 502 Piedras Negras Coahuil');
...
--
ALTER TABLE `alumnos`
  MODIFY `id_alumno` int(255) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;
